...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const errs = [];
      if (!pharmacyName?.trim()) errs.push("Pharmacy name is required");
      ...
      if (!zipCode?.trim()) {
        ...
        errs.push("Zip must be 5 digits (or 5+4)");
      }
      ...
      if (!fax || !/^\d{10}$/.test(fax)) {
        errs.push("Fax must be exactly 10 digits");
      }
      ...
      if (state && !VALID_STATES.has(String(state).toUpperCase())) {
        errs.push("Invalid state code");
      }

      if (errs.length > 0) {
        return res.status(400).json({ message: errs.join(", ") });
      }
