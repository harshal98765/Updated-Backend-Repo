...
export const requestJoinGroup = async (req, res) => {
  try {
    ...
    if (!group.is_discoverable)
      return res.status(403).json({ error: "This group is private and cannot be joined directly" });
    ...
    if (dupInv.rows.length > 0)
      return res.status(400).json({ error: "You already have a pending request or invitation for this group" });
...
export const updateGroup = async (req, res) => {
  try {
    ...
    if (adminCheck.rows.length === 0)
      return res.status(403).json({ error: "Only the group admin can update this group" });
  ...
  try {
    ...
    if (inv.direction === "admin_to_user") {
      ...
      const isForMe =
        ...
        (myEmail && inv.invited_email && inv.invited_email.toLowerCase() === myEmail.toLowerCase());
      if (!isForMe) return res.status(403).json({ error: "Not authorized" });
      if (!pharmacyId) return res.status(400).json({ error: "No pharmacy registered for this user" });
      memberPharmacyId = pharmacyId;
      memberUserId = userId;
    } else {
      // direction = 'user_to_admin' — I'm the admin approving the request
      if (inv.created_by_user_id !== userId)
        return res.status(403).json({ error: "Only the group admin can approve join requests" });
    ...
    if (Number(targetCount.rows[0].count) >= 5)
      return res.status(400).json({
        error: inv.direction === "admin_to_user"
  ...
  try {
    ...
    const repRes = await pool.query(
      `INSERT INTO inventory_group_reports
       ...
       RETURNING *`,
      [groupId, pharmacyId, userId, pharmacyName, source_audit_id, label, rows.length]
    );
...
export const deleteGroupReport = async (req, res) => {
  try {
    ...
    if (check.rows.length === 0)
      return res
        .status(403)
        .json({ error: "Only the report owner or group admin can delete this report" });

    await pool.query(`DELETE FROM inventory_group_reports WHERE id = $1`, [reportId]);
